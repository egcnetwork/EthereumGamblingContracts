...
import "./EGCBankroll.sol";
import "./SafeMath.sol";

contract EGCDice is usingOraclize, EGCGameInterface {
	...
	function getMaxWin() public view returns(uint256){
		return (SafeMath.mul(EGCBankrollInterface(BANKROLLER).getBankroll(), MAXWIN_inTHOUSANDTHPERCENTS) / 1000);
	}
	...
	function setBankrollerContractOnce(address bankrollAddress) public {
		...
		require(EGCBankrollInterface(bankrollAddress).getBankroll() != 0);
	...
	function play(uint256 betPerRoll, uint16 rolls, uint8 rollUnder) public payable {
		...
		EGCBankrollInterface(BANKROLLER).payOraclize(oraclize_getPrice('random', gasToSend));
	...
	function __callback(bytes32 _queryId, string _result, bytes _proof) public {
		...
		// else, resolve the bet as normal with this miner-proof proven-randomness from oraclize.
			...
			EGCBankrollInterface(BANKROLLER).receiveEtherFromGameAddress.value(SafeMath.sub(data.etherReceived, developersCut))();

			// force the bankroller contract to pay out the player
			EGCBankrollInterface(BANKROLLER).payEtherToWinner(etherAvailable, data.player);
...
// END OF CONTRACT. REPORT ANY BUGS TO DEVELOPMENT@EGC.IO
