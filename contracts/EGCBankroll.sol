...
contract EGCBankroll is ERC20, EGCBankrollInterface {

	using SafeMath for *;

	// constants for EGC Bankroll
	...
	string public constant name = "EGC Stake Tokens";
	...
	// checks that an address is a "trusted address of a legitimate EGC game"
	...
	function EGCBankroll(address dice, address slots) public payable {
	...
	function payOraclize(uint256 amountToPay) public addressInTrustedAddresses(msg.sender){
		...
		EGCGameInterface(msg.sender).receivePaymentForOraclize.value(amountToPay)();
	}
	...
	// this function ADDS to the bankroll of EGC, and credits the bankroller a proportional
	...
	function cashoutEGCStakeTokens(uint256 _amountTokens) public {
	...
	function cashoutEGCStakeTokens_ALL() public {

		// just forward to cashoutEGCStakeTokens with input as the senders entire balance
		cashoutEGCStakeTokens(balances[msg.sender]);
	}
	...
	function withdrawDevelopersFund(address receiver) public {
        ...
        EGCGameInterface(DICE).payDevelopersFund(receiver);
		EGCGameInterface(SLOTS).payDevelopersFund(receiver);
